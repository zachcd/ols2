  ...
  if (props.loggedIn) {
    return (
      <div className="nav">
        ...
        <div id="nav3 onClick={() => props.logout()}>Logout</div>
      </div>
    )
  } else {
    return (
...
const mapStateToProps = state => {
  return {loggedIn: state.user.status, currentOrg: state.organization}
}
...
const MainNav = connect(mapStateToProps, mapDispatchToProps)(Nav)
