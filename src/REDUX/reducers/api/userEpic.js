...
const registerEpic = action$ => action$.filter(action => action.type === REGISTER).mergeMap(action => {
  return ajax.post('http://localhost:3200/api/register', action.payload).map(ajx => {
...
const loginEpic = action$ => action$.filter(action => action.type === LOGIN).mergeMap(action => {
  return ajax.post('http://localhost:3200/api/login', action.payload).map(ajx => {
    console.log(ajx)
    if (ajx.response.message === "Success") {
