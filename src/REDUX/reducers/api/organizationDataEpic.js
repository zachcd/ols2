...
const loadOrganizationEpic = action$ => action$.filter(action => action.type === LOAD_ORGANIZATIONS).mergeMap(action => {
  return ajax.get('http://localhost:3200/api/organizations').map(ajx => {
...
const loadTournamentsEpic = action$ => action$.filter(action => action.type === LOAD_TOURNAMENTS).mergeMap(action => {
  return ajax.get('http://localhost:3200/api/organizations/' + action.payload.organization).map(ajx => {
    if (ajx.response.message === "Success") {
